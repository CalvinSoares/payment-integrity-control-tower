...
def parse_settlement_csv(
        ...
        rows.append(
            {
                ...
                "settledAt": _iso(str(row["settled_at"]).strip(), f"linha {row_number} settled_at"),
                "grossAmountMinor": _minor(str(row["gross_amount_minor"]).strip(), f"linha {row_number} gross_amount_minor", False),
                "feeAmountMinor": _minor(str(row["fee_amount_minor"]).strip(), f"linha {row_number} fee_amount_minor"),
                "netAmountMinor": _minor(str(row["net_amount_minor"]).strip(), f"linha {row_number} net_amount_minor"),
                "currency": currency,
            }
        )
    if not rows:
    ...
    return {
        ...
        "providerAccountId": provider_account_id.strip(),
        "fileName": file_name.strip(),
        "fileChecksum": checksum,
        "periodStart": start,
        "periodEnd": end,
        "receivedAt": _iso(received_at, "receivedAt"),
        "rows": rows,
        "fileKey": json.dumps([provider.strip(), provider_account_id.strip(), file_name.strip(), checksum, start, end], ensure_ascii=False, separators=(",", ":")),
    }


def _evidence(batch: dict[str, Any], row: dict[str, Any], payment: tuple[Any, ...] | None) -> list[str]:
    return [
        f"settlement:{batch['batchId']}:{row['rowNumber']}",
        f"settlement-record:{row['settlementId']}",
        *([] if payment is None else [f"payment:{payment[0]}"]),
    ]


def _map_run(row: tuple[Any, ...]) -> dict[str, Any]:
    return {
        "runId": row[0], "tenantId": row[1], "batchId": row[2], "ruleVersion": row[3],
        "idempotencyKey": row[4], "status": row[5], "createdAt": row[6].isoformat(),
        "completedAt": row[7].isoformat(), "itemCount": row[8], "exceptionCount": row[9],
    }


def _map_item(row: tuple[Any, ...]) -> dict[str, Any]:
    result: dict[str, Any] = {
        "itemId": row[0], "runId": row[1], "batchId": row[2], "rowNumber": row[3],
        "settlementId": row[4], "externalPaymentId": row[5], "status": row[7],
        "currency": str(row[13]).strip(), "evidence": row[14], "ruleVersion": row[15],
    }
    if row[6] is not None: result["paymentId"] = row[6]
    if row[8] is not None: result["category"] = row[8]
    if row[9] is not None: result["severity"] = row[9]
    for key, index in (("expectedMinor", 10), ("observedMinor", 11), ("differenceMinor", 12)):
        if row[index] is not None: result[key] = int(row[index])
    return result


def _map_exception(row: tuple[Any, ...]) -> dict[str, Any]:
    result: dict[str, Any] = {
        "exceptionId": row[0], "tenantId": row[1], "runId": row[2], "itemId": row[3],
        "batchId": row[4], "category": row[5], "severity": row[6], "status": row[7],
        "currency": str(row[11]).strip(), "evidence": row[12], "ruleVersion": row[13],
        "reprocessable": row[14], "createdAt": row[15].isoformat(),
    }
    for key, index in (("expectedMinor", 8), ("observedMinor", 9), ("differenceMinor", 10)):
        if row[index] is not None: result[key] = int(row[index])
    if row[16] is not None: result["resolvedAt"] = row[16].isoformat()
    if row[17] is not None: result["resolution"] = row[17]
    return result
...
class PostgresSettlementService:
    ...
    def receive_csv(self, **input_data: str) -> dict[str, Any]:
        ...
        with psycopg.connect(self.database_url) as connection, connection.transaction(), connection.cursor() as cursor:
            cursor.execute("SELECT rows_json FROM settlement_batches WHERE file_key = %s", (batch["fileKey"],))
            existing = cursor.fetchone()
            if existing is not None:
                return {"status": "REPLAYED", "batchId": batch["batchId"], "rowCount": len(existing[0])}
            cursor.execute(
                """
                ...
                """,
                (batch["batchId"], batch["tenantId"], batch["provider"], batch["providerAccountId"], batch["fileName"],
                 batch["fileChecksum"], batch["fileKey"], batch["periodStart"], batch["periodEnd"], batch["receivedAt"],
                 Jsonb(batch["rows"]), len(batch["rows"])),
            )
        return {"status": "RECEIVED", "batchId": batch["batchId"], "rowCount": len(batch["rows"])}
            ...
            for row in batch_row[1]:
                ...
                if payment is not None and not issues:
                    ...
                    if row["netAmountMinor"] + row["feeAmountMinor"] != row["grossAmountMinor"]:
                        issues.append({"category": "FEE_MISMATCH", "expectedMinor": row["grossAmountMinor"], "observedMinor": row["netAmountMinor"] + row["feeAmountMinor"], "differenceMinor": row["netAmountMinor"] + row["feeAmountMinor"] - row["grossAmountMinor"]})
                ...
                evidence = _evidence({"batchId": batch_id}, row, payment)
                issue_list = issues or [{"category": None}]
                for issue in issue_list:
                    category = issue["category"]
                    item_id = f"{run_id}:{row['rowNumber']}:{category or 'MATCHED'}"
                    item = {"itemId": item_id, "runId": run_id, "batchId": batch_id, "rowNumber": row["rowNumber"], "settlementId": row["settlementId"], "externalPaymentId": row["externalPaymentId"], "status": "EXCEPTION" if category else "MATCHED", "currency": row["currency"], "evidence": evidence, "ruleVersion": rule_version}
                    if payment is not None: item["paymentId"] = payment[0]
