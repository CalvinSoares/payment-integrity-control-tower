...
from payment_integrity.analytics.projector import EventAnalyticsProjector
...
class PostgresEventWorker:
    ...
    def log_event(event: PaymentEvent, _connection: psycopg.Connection[Any]) -> None:
        logger.info("event_processed event_id=%s event_type=%s tenant_id=%s", event.event_id, event.event_type, event.tenant_id)
    ...
    def process_next(self, now: datetime | None = None) -> WorkerResult:
        ...
        with psycopg.connect(self.database_url) as connection:
            with connection.transaction():
                ...
                event = PaymentEvent.from_mapping(event_json)
                ...
                except Exception as error:  # noqa: BLE001 - handler failures enter retry/DLQ
        ...
        if projected_event is not None and self.projector is not None:
            ...
            except Exception:  # noqa: BLE001 - derived analytics cannot undo financial state
                logger.exception("analytics_projection_failed event_id=%s", projected_event.event_id)
        return result
    ...
    def _schedule_failure(self, connection: psycopg.Connection[Any], outbox_id: str, inbox_id: str, event_id: str, now_iso: str, attempts: int, error: Exception) -> WorkerResult:
        ...
        return WorkerResult(status="RETRY_SCHEDULED", event_id=event_id, error=error_text, next_attempt_at=next_attempt, attempts=attempts)

    def _recover_stale_processing(self, connection: psycopg.Connection[Any], now_iso: str) -> None:
        with connection.cursor() as cursor:
            cursor.execute("UPDATE event_outbox SET status = 'PENDING', available_at = %s::timestamptz, processing_started_at = NULL, last_error = 'Lease de processamento expirado.' WHERE status = 'PROCESSING' AND processing_started_at <= (%s::timestamptz - (%s::text || ' milliseconds')::interval)", (now_iso, now_iso, self.lease_ms))

    @staticmethod
    def _claim_next(connection: psycopg.Connection[Any], now_iso: str) -> tuple[str, str, dict[str, Any], int] | None:
        with connection.cursor() as cursor:
            cursor.execute("WITH next_message AS (SELECT outbox_id FROM event_outbox WHERE status = 'PENDING' AND available_at <= %s::timestamptz ORDER BY available_at, outbox_id FOR UPDATE SKIP LOCKED LIMIT 1) UPDATE event_outbox AS outbox SET status = 'PROCESSING', attempts = outbox.attempts + 1, processing_started_at = %s::timestamptz FROM next_message WHERE outbox.outbox_id = next_message.outbox_id RETURNING outbox.outbox_id, outbox.event_id, outbox.event_json, outbox.attempts", (now_iso, now_iso))
            row = cursor.fetchone()
    ...
    def _mark_inbox_processing(connection: psycopg.Connection[Any], inbox_id: str) -> None:
        with connection.cursor() as cursor:
            cursor.execute("UPDATE event_inbox SET status = 'PROCESSING', attempts = attempts + 1 WHERE inbox_id = %s", (inbox_id,))

    @staticmethod
    def _mark_inbox_applied(connection: psycopg.Connection[Any], inbox_id: str, processed_at: str) -> None:
        with connection.cursor() as cursor:
            cursor.execute("UPDATE event_inbox SET status = 'APPLIED', processed_at = %s::timestamptz WHERE inbox_id = %s", (processed_at, inbox_id))
    ...
    def _mark_outbox_published(connection: psycopg.Connection[Any], outbox_id: str, published_at: str) -> None:
        with connection.cursor() as cursor:
            cursor.execute("UPDATE event_outbox SET status = 'PUBLISHED', published_at = %s::timestamptz, processing_started_at = NULL WHERE outbox_id = %s", (published_at, outbox_id))

    @staticmethod
    def _mark_outbox_failed(connection: psycopg.Connection[Any], outbox_id: str, error: str) -> None:
        with connection.cursor() as cursor:
            cursor.execute("UPDATE event_outbox SET status = 'FAILED', last_error = %s, processing_started_at = NULL WHERE outbox_id = %s", (error, outbox_id))

    @staticmethod
    def _schedule_outbox(connection: psycopg.Connection[Any], outbox_id: str, available_at: str, error: str, dead_letter: bool) -> None:
        with connection.cursor() as cursor:
            cursor.execute("UPDATE event_outbox SET status = %s, available_at = %s::timestamptz, last_error = %s, processing_started_at = NULL WHERE outbox_id = %s", ("DEAD_LETTER" if dead_letter else "PENDING", available_at, error, outbox_id))
