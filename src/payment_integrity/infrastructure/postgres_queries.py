"""PostgreSQL read adapter for the control tower."""
...
def _map_payment(row: tuple[object, ...]) -> dict[str, object]:
    return {
        "id": row[0],
        "tenantId": row[1],
...
def _map_journals(rows: Sequence[tuple[object, ...]]) -> list[dict[str, object]]:
    ...
    for row in rows:
        ...
        if not isinstance(lines, list):
            raise ValueError("ledger journal lines inválidas")
        lines.append(
            {
                "lineId": row[3],
                "accountId": row[4],
...
def _map_exception(row: tuple[object, ...]) -> dict[str, object]:
...
class PostgresControlTowerQueries:
    """Tenant-scoped implementation of the application query port."""

    def __init__(self, database_url: str) -> None:
        self.database_url = database_url

    def _payment(self, cursor: psycopg.Cursor[tuple[object, ...]], payment_id: str, tenant_id: str) -> dict[str, object] | None:
        cursor.execute(
            """
    ...
    @staticmethod
    def _ledger(cursor: psycopg.Cursor[tuple[object, ...]], payment_id: str) -> list[dict[str, object]]:
        cursor.execute(
            """
    ...
    @staticmethod
    def _exceptions_for_payment(
        cursor: psycopg.Cursor[tuple[object, ...]], tenant_id: str, external_payment_id: str, payment_id: str
    ...
    def get_payment_timeline(self, payment_id: str, tenant_id: str) -> dict[str, object] | None:
        with psycopg.connect(self.database_url) as connection, connection.cursor() as cursor:
            ...
            return {
                ...
                "exceptions": self._exceptions_for_payment(cursor, tenant_id, str(payment["externalPaymentId"]), payment_id),
            }
